...
class Word_Soup:
    ...
    def entry_analyzer(self):
        ...
        if action_id.buyin_approve(self.entry):
            self.ans,self.actor,self.action,self.stack = action_id.buyin_approve(self.self.entry)
        ...
        return self.action, self.actor, self.stack, self.player_s,self.handnumber,self.blind,self.amount
